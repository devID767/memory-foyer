...
- Deck cards are spawned dynamically — one card per deck (the repository is the local source of truth for which decks exist; per-deck `due / total` counts come from the side-effect-free `GET /decks` aggregate). Decks are ordered deterministically — **due-gate** first (decks with any due cards rank above caught-up ones), then **`deckId` ordinal** tiebreak — then capped by `FoyerLayoutConfig.MaxDecksToShow` and laid out horizontally with configurable spacing. So when there are more decks than slots, the ones needing work are shown, in a stable order that does not reshuffle day to day.
