...
- **Screen aggregators are forwarders, not coordinators.** `FoyerScreen` and `ReviewScreen` group leaf views and proxy operations to them; they do not inject application services, hold business state, or sequence multiple leaf awaits. They MAY receive an injected animation-config ScriptableObject (`UIAnimationConfig`), own a `CanvasTransition` for their own canvas root, and push that config down to leaf views (`_grades.Configure` / `_summary.Configure`) — configuration wiring, not coordination. Each `*Async` method on a screen returns a single leaf's `UniTask`. The one documented exception: `Show()` and `Hide()` own the screen's own canvas-visibility — `Show()` MAY synchronously reset stale leaf visibility (hide grade buttons / summary) so a re-entered session starts from a clean baseline, then fades the canvas in via its `CanvasTransition` (fire-and-forget, not an awaited leaf sequence); `Hide()` kills that transition and deactivates the canvas instantly. Anything beyond that — multi-await sequencing, decisions based on application state — belongs in the presenter.
